		...
		(set, get) => ({
			...
			signInWithEmail: async (email, password) => {
				...
				try {
					const timeout = new Promise<never>((_, reject) =>
						setTimeout(
							() =>
								reject(
									new Error(
										"Koneksi ke Firebase lambat. Cek koneksi internet Anda atau coba lagi.",
									),
								),
							LOGIN_TIMEOUT_MS,
						),
					...
					const { user } = (await Promise.race([signIn, timeout])) as Awaited<
						typeof signIn
					>;
					const { ensureUserProfileFromAuth } =
						await loadFirestoreAuthHelpers();
			...
			requestPasswordReset: async (email) => {
				...
				try {
					await sendPasswordResetEmail(auth, email, {
						...
						// Arahkan flow reset ke route app agar tidak jatuh ke halaman handler Firebase default.
			...
			refreshUser: async () => {
				...
				const { getUserProfile, ensureUserProfileFromAuth } =
					await loadFirestoreAuthHelpers();
