...
export default function ImportsPage() {
	...
	const expenseCategoryOptions = useMemo(
		() => buildCategoryOptions(categories, "expense"),
		[categories],
	);
	...
	const [analysis, setAnalysis] = useState<ReceiptAnalysisResponse | null>(
		null,
	);
	...
	const [importBank, setImportBank] =
		useState<(typeof IMPORT_SOURCES)[number]["value"]>("bca");
	...
	const handleAnalyze = async () => {
		...
		if (!isBackendConfigured) {
			setReceiptError(backendRequiredMessage);
			return;
		}
		...
		try {
			...
			const { data } = await uploadApi.post<ReceiptAnalysisResponse>(
				"/ai/receipt",
				formData,
				{
					headers: {
			...
			const normalizedCategory = selectableExpenseCategories.some(
			...
			setDraft((prev) => ({
				...
				note: data.items?.length
					? `OCR struk: ${data.items.length} item terbaca`
					: prev.note,
			}));
		} catch (err: any) {
	...
	const handleDraftChange = <K extends keyof ReceiptDraft>(
		field: K,
	...
	const handleSaveReceipt = async () => {
		if (!draft.wallet_id || draft.amount <= 0) {
			setReceiptError(
				"Pastikan nominal dan wallet sudah terisi sebelum menyimpan.",
			);
			return;
		}
	...
	const handlePreviewImport = async () => {
		...
		if (!isBackendConfigured) {
			setImportError(backendRequiredMessage);
			return;
		}
		...
		try {
			...
			const { data } = await uploadApi.post<ImportPreviewResponse>(
				"/imports/preview",
				formData,
				{
					headers: {
	...
	const handleConfirmImport = async () => {
		...
		if (!previewData || !importWalletId) {
			setImportError(
				"Pilih wallet dan preview data dulu sebelum konfirmasi import.",
			);
			return;
		}

		const chosenRows = importRows.filter((row) =>
			selectedHashes.includes(row.hash),
		);
		...
		try {
			const { data } = await api.post<ConfirmImportResponse>(
				"/imports/confirm",
				{
			...
			>
				...
				>
					...
					>
						...
						>
							Flow ini dipakai untuk foto nota belanja dan fallback review
							manual jika OCR confidence rendah.
						</div>
					</label>
					...
					{previewUrl ? (
						...
						>
							Preview PDF belum ditampilkan di halaman ini, tapi file tetap bisa
							dikirim ke OCR backend.
						</div>
					) : null}
					...
					{receiptError ? (
						<StatusBox tone="danger" message={receiptError} />
					) : null}
					{receiptSuccess ? (
						<StatusBox tone="success" message={receiptSuccess} />
					) : null}
				</section>
					...
					{!analysis ? (
						...
						<>
							<div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
								<span
									className={`badge ${analysis.readable ? "badge-ok" : "badge-danger"}`}
								>
									{analysis.readable ? "Struk terbaca" : "Perlu review manual"}
								</span>
								<span
									className={`badge ${analysis.confidence >= 0.7 ? "badge-ok" : analysis.confidence >= 0.5 ? "badge-warn" : "badge-danger"}`}
								>
							...
							>
								<Field label="Nominal">
									<input
										...
										value={draft.amount}
										onChange={(event) =>
											handleDraftChange("amount", Number(event.target.value))
										}
									/>
								</Field>
								...
								<Field label="Merchant">
									<input
										...
										onChange={(event) =>
											handleDraftChange("merchant", event.target.value)
										}
								...
								<Field label="Wallet">
									<select
										...
										onChange={(event) =>
											handleDraftChange("wallet_id", event.target.value)
										}
									>
							...
							<button
								...
								disabled={isSaving || !draft.wallet_id || draft.amount <= 0}
							>
			...
			>
				...
				>
					...
					>
						<Field label="Sumber bank / dompet digital">
							<select
								...
								onChange={(event) =>
									setImportBank(
										event.target
											.value as (typeof IMPORT_SOURCES)[number]["value"],
									)
								}
							>
						...
						<Field label="Wallet tujuan">
							<select
								...
								onChange={(event) => setImportWalletId(event.target.value)}
							>
								<option value="">Pilih wallet</option>
								{activeWallets.map((wallet) => (
									<option key={wallet.id} value={wallet.id}>
					...
					>
						...
						>
							Parser backend sudah mendukung BCA, Mandiri, BNI, BRI, GoPay, dan
							OVO.
						</div>
					</label>
					...
					{importError ? (
						<StatusBox tone="danger" message={importError} />
					) : null}
					{importSuccess ? (
						<StatusBox tone="success" message={importSuccess} />
					) : null}
					...
					{!previewData ? (
						...
						<>
							...
							>
								<MiniStat
									label="Total baris"
									value={String(previewData.total_rows)}
								/>
								<MiniStat label="Valid" value={String(previewData.imported)} />
								<MiniStat
									label="Duplikat"
									value={String(previewData.duplicates.length)}
								/>
								<MiniStat
									label="Error"
									value={String(previewData.errors.length)}
								/>
							</div>
								...
								<div style={{ maxHeight: "340px", overflowY: "auto" }}>
									{importRows.map((row) => (
										...
										>
											<input
												type="checkbox"
												checked={selectedHashes.includes(row.hash)}
												onChange={() => handleToggleRow(row.hash)}
											/>
											<span
												style={{
													fontSize: "12px",
											...
											<div style={{ minWidth: 0 }}>
												...
												>
													<span
														className={`badge ${row.is_duplicate ? "badge-warn" : "badge-ok"}`}
													>
											...
											<span
												className={`badge ${row.type === "income" ? "badge-ok" : "badge-danger"}`}
											>
				...
				>
					...
					/>
					<ChecklistItem
						text="Skip/import pilihan user saat confirm sudah aktif."
						done
					/>
...
function MiniStat({ label, value }: { label: string; value: string }) {
	return (
		<div
			className="card"
			style={{ padding: "12px 14px", background: "var(--bg-card2)" }}
		>
			<div
				style={{
					fontSize: "11px",
