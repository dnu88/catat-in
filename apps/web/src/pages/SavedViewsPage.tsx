...
export default function SavedViewsPage() {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const res = await api.get('/saved-views')
  ...
  const create = async (e: FormEvent) => {
    ...
    setError('')
    try {
      await api.post('/saved-views', { name, scope, filters: JSON.parse(filters || '{}') })
      setName('')
      await load()
    } catch (err: any) {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'grid', gap: 12 }}>
      ...
      {isLoading ? <div className="card" style={{ padding: 12 }}>{language === 'id' ? 'Memuat saved views...' : 'Loading saved views...'}</div> : null}

      <div style={{ display: 'grid', gap: 8 }}>
        {!isLoading && items.length === 0 ? <div className="card" style={{ padding: 16, textAlign: 'center', color: 'var(--text-muted)' }}>{language === 'id' ? 'Belum ada saved view.' : 'No saved views yet.'}</div> : null}
        {items.map((item) => (
          <div key={item.id} className="card" style={{ padding: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
            <div>
