...
const RECURRENCE_LABEL: Record<string, string> = {
  once: 'Sekali', monthly: 'Bulanan', yearly: 'Tahunan',
}

// ── ADD BILL MODAL ────────────────────────────────────────────

function AddBillModal({ onClose, onSave }: { onClose: () => void; onSave: (d: BillFormData) => Promise<void> }) {
  const [name, setName] = useState('')
  const [amount, setAmount] = useState('')
  const [dueDay, setDueDay] = useState('1')
  const [recurrence, setRecurrence] = useState<BillFormData['recurrence']>('monthly')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await onSave({ name, amount: parsedAmount, due_day: parseInt(dueDay), recurrence, icon, notify_before_days: [3, 1] })
      onClose()
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} style={modalStyles.form}>
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            <div style={{ flex: '0 0 auto' }}>
              ...
              <input style={{ ...modalStyles.input, width: '60px', textAlign: 'center', fontSize: '1.2rem' }}
            ...
            <div style={{ flex: 1 }}>
              ...
              <input style={modalStyles.input} value={name} onChange={(e) => setName(e.target.value)}
                placeholder="cth: Listrik PLN" required />
            </div>
          </div>

          <label style={modalStyles.label}>Nominal (Rp)</label>
          <input style={modalStyles.input} type="number" min="1000" step="1000" value={amount}
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            <div style={{ flex: 1 }}>
              ...
              <input style={modalStyles.input} type="number" min="1" max="31" value={dueDay}
                onChange={(e) => setDueDay(e.target.value)} required />
            </div>
            <div style={{ flex: 1 }}>
              <label style={modalStyles.label}>Perulangan</label>
              <select style={modalStyles.input} value={recurrence}
                onChange={(e) => setRecurrence(e.target.value as BillFormData['recurrence'])}>
...
function BillCard({ bill, onPay, onDelete }: { bill: BillReminder; onPay: (id: string) => void; onDelete: (id: string) => void }) {
  const days = daysUntil(bill.next_due_date)
  const isOverdue = days < 0
  const isSoon = days >= 0 && days <= 3

  return (
    <div style={{ ...cardStyles.card, borderLeft: `4px solid ${isOverdue ? '#dc2626' : isSoon ? '#f59e0b' : '#2563eb'}` }}>
      <div style={cardStyles.header}>
        <div style={cardStyles.iconName}>
          <span style={cardStyles.icon}>{bill.icon || '📄'}</span>
          <div>
            <p style={cardStyles.name}>{bill.name}</p>
            <p style={cardStyles.recurrence}>{RECURRENCE_LABEL[bill.recurrence]}</p>
          </div>
        </div>
        <button onClick={() => onDelete(bill.id)} style={cardStyles.deleteBtn} title="Hapus">✕</button>
      </div>
      <div style={cardStyles.footer}>
        <div>
          <p style={cardStyles.amount}>{formatRupiah(Number(bill.amount))}</p>
          <p style={{ ...cardStyles.dueInfo, color: isOverdue ? '#dc2626' : isSoon ? '#d97706' : '#6b7280' }}>
            {isOverdue
...
export default function BillsPage() {
  const { bills, isLoading, error, fetchBills, addBill, payBill, deleteBill } = useBillsStore()
  ...
  const handlePay = async (id: string) => {
    ...
    try { await payBill(id) } catch { alert('Gagal memproses pembayaran') }
  }

  const handleDelete = async (id: string) => {
    if (!window.confirm('Hapus tagihan ini?')) return
    try { await deleteBill(id) } catch { alert('Gagal menghapus tagihan') }
  }

  const overdueCount = bills.filter((b) => daysUntil(b.next_due_date) < 0).length
  const soonCount = bills.filter((b) => { const d = daysUntil(b.next_due_date); return d >= 0 && d <= 3 }).length

  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <div>
          <h2 style={styles.title}>Tagihan & Cicilan</h2>
          {(overdueCount > 0 || soonCount > 0) && (
            <p style={styles.alertText}>
              {overdueCount > 0 && <span style={{ color: '#dc2626' }}>{overdueCount} terlambat </span>}
              {soonCount > 0 && <span style={{ color: '#d97706' }}>{soonCount} jatuh tempo dalam 3 hari</span>}
            </p>
          )}
        </div>
        <button onClick={() => setShowModal(true)} style={styles.addBtn}>+ Tambah Tagihan</button>
      </div>
      ...
      {!isLoading && bills.length === 0 && (
        <div style={styles.empty}>
          ...
          <p>Belum ada tagihan. Tambahkan pengingat tagihan agar tidak terlewat!</p>
        </div>
      )}

      <div style={styles.grid}>
        {bills.map((b) => <BillCard key={b.id} bill={b} onPay={handlePay} onDelete={handleDelete} />)}
      </div>

      {showModal && <AddBillModal onClose={() => setShowModal(false)} onSave={addBill} />}
    </div>
  )
}
...
const cardStyles: Record<string, React.CSSProperties> = {
  ...
  name: { margin: '0 0 0.15rem', fontSize: '0.95rem', fontWeight: 700, color: '#1a1a1a' },
  recurrence: { margin: 0, fontSize: '0.75rem', color: '#9ca3af' },
  deleteBtn: { background: 'none', border: 'none', cursor: 'pointer', color: '#d1d5db', fontSize: '0.85rem' },
  footer: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' },
  amount: { margin: '0 0 0.15rem', fontSize: '1.1rem', fontWeight: 700, color: '#1a1a1a' },
  dueInfo: { margin: 0, fontSize: '0.8rem' },
  payBtn: { padding: '0.5rem 1rem', backgroundColor: '#16a34a', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600 },
}
...
const modalStyles: Record<string, React.CSSProperties> = {
  ...
  box: { backgroundColor: 'white', borderRadius: '12px', padding: '2rem', width: '100%', maxWidth: '440px', boxShadow: '0 4px 24px rgba(0,0,0,0.15)' },
  title: { margin: '0 0 1.25rem', fontSize: '1.2rem', color: '#1a1a1a' },
  form: { display: 'flex', flexDirection: 'column', gap: '0.75rem' },
  label: { fontSize: '0.85rem', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '0.25rem' },
