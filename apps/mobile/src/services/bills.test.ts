...
describe('Bill Service', () => {
  ...
  test('createBill should respect custom notify_before_days', async () => {
    ...
    await createBill({ name: 'Rent', amount: 2000000, due_day: 1, recurrence: 'monthly', next_due_date: '2026-06-01', notify_before_days: 7 });

    expect(mockInsert).toHaveBeenCalledWith(expect.objectContaining({ notify_before_days: 7 }));
  });
  ...
  test('listBills should return array ordered by next_due_date asc', async () => {
    ...
    const mockOrder = jest.fn().mockResolvedValue({ data: mockBills, error: null });
    ...
    expect(mockOrder).toHaveBeenCalledWith('next_due_date', { ascending: true });
  ...
  test('updateBill should call update/eq/select/single and return updated record', async () => {
    ...
    expect(supabase.from).toHaveBeenCalledWith('bill_reminders');
    expect(mockUpdate).toHaveBeenCalledWith({ is_paid: true });
  ...
  test('createBill should throw when Supabase returns error', async () => {
    ...
    await expect(createBill({ name: 'Fail', amount: 0, due_day: 1, recurrence: 'once', next_due_date: '2026-05-01' })).rejects.toEqual(mockError);
  });
});
