      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <SectionHeader
          ...
          action={
            <View style={styles.headerActions}>
              <Pressable style={styles.addButton} onPress={() => setActiveForm(activeForm === 'create' ? null : 'create')}>
                <Text style={styles.addButtonText}>Buat</Text>
              </Pressable>
              <Pressable style={styles.secondaryButton} onPress={() => setActiveForm(activeForm === 'join' ? null : 'join')}>
        ...
        <Card variant="muted" style={styles.infoCard}>
          ...
          <View style={styles.infoContent}>
            ...
            <Text style={styles.infoSub}>Buat ruang bersama, undang anggota, dan pantau akses dasar tiap keluarga.</Text>
          </View>
        </Card>
        ...
        {activeForm === 'create' ? (
          <Card variant="default" style={styles.formCard}>
            ...
            <TextInput
              ...
              style={styles.input}
            />
            <Pressable style={[styles.submitButton, submitting && styles.disabledButton]} onPress={handleCreate} disabled={submitting}>
        ...
        {activeForm === 'join' ? (
          <Card variant="default" style={styles.formCard}>
            ...
            <TextInput
              ...
              style={styles.input}
            />
            <Pressable style={[styles.submitButton, submitting && styles.disabledButton]} onPress={handleJoin} disabled={submitting}>
        ...
        {loading ? <Text style={styles.emptyText}>Memuat keluarga...</Text> : null}

        {!loading && memberships.length === 0 ? (
          <Card variant="default" style={styles.emptyCard}>
            <IconBubble name="groups" tone="accent" size={46} />
            <Text style={styles.emptyTitle}>Belum ada keluarga</Text>
            <Text style={styles.emptyText}>Buat keluarga baru atau gabung memakai kode undangan.</Text>
          </Card>
        ) : null}
        ...
        {memberships.map((membership) => {
          ...
          return (
            <Card key={membership.id ?? membership.household_id} variant="default" style={styles.groupCard}>
              <View style={styles.groupTop}>
                <View style={styles.groupLeft}>
                  <IconBubble name="groups" tone={membership.role === 'viewer' ? 'accent' : 'primary'} size={46} />
                  <View style={styles.groupText}>
                    <Text style={styles.groupName}>{household?.name ?? 'Keluarga'}</Text>
                    <Text style={styles.groupMeta}>Anggota</Text>
                  </View>
                </View>
                <View style={[styles.roleBadge, membership.role !== 'viewer' && styles.roleBadgeActive]}>
                  <Text style={[styles.roleBadgeText, membership.role !== 'viewer' && styles.roleBadgeTextActive]}>{roleLabel}</Text>
                </View>
              </View>
              ...
              <View style={styles.groupBottom}>
                <View>
                  ...
                  <Text style={styles.balanceValue}>{household?.invite_code ?? '-'}</Text>
                </View>
...
function createStyles(theme: ReturnType<typeof useTheme>['theme']) {
  return StyleSheet.create({
    ...
    secondaryButtonText: { color: theme.colors.brandPrimary, fontSize: 12, fontWeight: '800' },
    ...
    submitButtonText: { color: theme.colors.textInverse, fontSize: 13, fontWeight: '800' },
    ...
    memberLabel: { color: theme.colors.textSecondary, fontSize: 12, fontWeight: '800' },
  })
}
